    ...
    try {
        ...
        await db.query(`
            CREATE TABLE IF NOT EXISTS dashboard_stats (
                ...
                chc_count INT,
                today_patients INT,
                doctors_on_duty_percentage INT,
                ai_prediction_score_percentage INT,
                ambulance_availability_ratio VARCHAR(50),
                critical_alerts INT
            )
        `);
        ...
        if (statsCheck[0].count === 0 && seedData.dashboard_stats) {
            ...
            await db.query(`INSERT INTO dashboard_stats 
                (id, phc_count, chc_count, today_patients, doctors_on_duty_percentage, ai_prediction_score_percentage, ambulance_availability_ratio, critical_alerts) 
                VALUES (1, ?, ?, ?, ?, ?, ?, ?)`, 
            [s.phc_count, s.chc_count, s.today_patients, s.doctors_on_duty_percentage, s.ai_prediction_score_percentage, s.ambulance_availability_ratio, s.critical_alerts]);
